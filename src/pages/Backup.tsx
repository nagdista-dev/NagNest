...
import { DatabaseBackup, Download, Upload, HardDrive, RotateCcw } from 'lucide-react'
...
export function Backup() {
  const { sites, categories, importData, resetAll, notify } = useSites()
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          ...
          <button
            ...
            className="mt-auto inline-flex items-center justify-center gap-1.5 rounded-xl bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-teal-700"
          >
            <Download size={15} />
            Download backup
          </button>
        ...
        <div className="flex flex-col gap-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              Restore from a JSON backup file. This replaces your current data.
            </p>
          </div>
          <button
            onClick={() => fileRef.current?.click()}
            className="mt-auto inline-flex items-center justify-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700"
          >
            <Upload size={15} />
            Choose file…
          </button>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-rose-100 bg-rose-50/60 p-5">
        ...
        <button
          ...
          className="self-start rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-rose-700"
        >
          Reset all data
        </button>
      </section>
